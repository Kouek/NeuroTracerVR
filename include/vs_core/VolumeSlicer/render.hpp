...
public:
	...
	// avoid via-CPU transfer like GetFrame()
    virtual bool RegisterGLPBOInCUDA(GLuint) = 0;
    virtual void UnregisterGLPBOInCUDA() = 0;
};

class VS_EXPORT CUDACompVolumeSubregionRenderer : public ICompVolumeRenderer {
public:
    static std::unique_ptr<CUDACompVolumeSubregionRenderer> Create(int w, int h, CUcontext ctx = nullptr);

	// avoid via-CPU transfer like GetFrame()
	virtual void RegisterGLPBOInCUDA(GLuint) = 0;
    virtual void UnregisterGLPBOInCUDA() = 0;

	virtual void Register3DPosOfCurosr(std::array<float, 4> &, const std::array<uint32_t, 2> &) = 0;
    virtual void Unregister3DPosOfCurosr() = 0;

	virtual void SetSubregion(const std::array<uint32_t, 3> &startVoxel, const std::array<uint32_t, 3> &voxelDim) = 0;

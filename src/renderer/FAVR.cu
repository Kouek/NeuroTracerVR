...
cudaArray_t d_subsampleTexArr2[2];
cudaTextureObject_t d_subsampleTex2[2];
__constant__ cudaTextureObject_t dc_subsampleTex2[2];
...
__global__ void createSubsampleTexKernel(uint2 *d_output, uint8_t FAVRLvl,
    ...
    for (uint8_t stage = 0; stage < FAVRLvl; ++stage) {
        ...
        if (sqr < lowerSqr || sqr > (stage == FAVRLvl - 1 ? INFINITY : hfWSqr))
            ...
            d0 = hfW * stage * d0;
            glm::vec2 targetPx = {floorf(fullResHfSz + d0.x + d.x),
                                  floorf(fullResHfSz + d0.y + d.y)};
...
void VRRenderer::createSubsampleAndReconsTex() {
    ...
    for (uint8_t lr = 0; lr < 2; ++lr) {
        if (d_subsampleTexArr2[lr] != nullptr)
            CUDA_RUNTIME_API_CALL(cudaFreeArray(d_subsampleTexArr2[lr]));
        cudaChannelFormatDesc chnnlDesc = cudaCreateChannelDesc<uchar4>();
        CUDA_RUNTIME_API_CALL(cudaMallocArray(
            &d_subsampleTexArr2[lr], &chnnlDesc, renderSz.x, renderSz.y));

        resDesc.res.array.array = d_subsampleTexArr2[lr];
        if (d_subsampleTexArr2[lr] != 0)
            CUDA_RUNTIME_API_CALL(
                cudaDestroyTextureObject(d_subsampleTex2[lr]));
        CUDA_RUNTIME_API_CALL(cudaCreateTextureObject(
            &d_subsampleTex2[lr], &resDesc, &texDesc, nullptr));
    }
    CUDA_RUNTIME_API_CALL(cudaMemcpyToSymbol(dc_subsampleTex2, d_subsampleTex2,
                                             sizeof(cudaTextureObject_t) * 2));
    ...
    for (uint8_t lvl = 1; lvl <= MAX_SUBSAMPLE_LEVEL; ++lvl) {
        ...
        resDesc.res.array.array = d_subsampleLookupTexArrs[idx];
        if (d_subsampleLookupTexes[idx] != 0)
            CUDA_RUNTIME_API_CALL(
                cudaDestroyTextureObject(d_subsampleLookupTexes[idx]));
        CUDA_RUNTIME_API_CALL(cudaCreateTextureObject(
            &d_subsampleLookupTexes[idx], &resDesc, &texDesc, nullptr));
    ...
    for (uint8_t lvl = 1; lvl <= MAX_SUBSAMPLE_LEVEL; ++lvl) {
        ...
        resDesc.res.array.array = d_reconsLookupTexArrs[idx];
        if (d_reconsLookupTexes[idx] != 0)
            CUDA_RUNTIME_API_CALL(
                cudaDestroyTextureObject(d_reconsLookupTexes[idx]));
        CUDA_RUNTIME_API_CALL(cudaCreateTextureObject(
            &d_reconsLookupTexes[idx], &resDesc, &texDesc, nullptr));

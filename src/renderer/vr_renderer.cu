...
static cudaArray_t d_preIntTFArray = nullptr;
...
cudaTextureObject_t d_clipPosTex2[2];
__constant__ cudaTextureObject_t dc_clipPosTex2[2];

size_t outputSize;
glm::u8vec4 *d_output2[2] = {nullptr, nullptr};
cudaGraphicsResource_t outputTexRsrc2[2] = {nullptr, nullptr};

void VRRenderer::SetCUDAxGLParam(const CUDAxGLParam &param) {
    size_t pxNum = param.renderSz.x * param.renderSz.y;
    outputSize = sizeof(glm::u8vec4) * pxNum;
    for (uint8_t lr = 0; lr < 2; ++lr) {
        if (d_output2[lr] == nullptr) {
            CUDA_RUNTIME_API_CALL(cudaMalloc(&d_output2[lr], outputSize));
        } else {
            CUDA_RUNTIME_API_CALL(cudaFree(d_output2[lr]));
            CUDA_RUNTIME_API_CALL(cudaMalloc(&d_output2[lr], outputSize));
            CUDA_RUNTIME_API_CALL(
                cudaGraphicsUnregisterResource(outputTexRsrc2[lr]));
        }
        CUDA_RUNTIME_API_CALL(cudaGraphicsGLRegisterImage(
            &outputTexRsrc2[lr], param.outputTex2[lr], GL_TEXTURE_2D,
...
void VRRenderer::SetProjectionParam(const ProjectionParam &param) {
    CUDA_RUNTIME_API_CALL(
        cudaMemcpyToSymbol(dc_projectionParam, &param, sizeof(ProjectionParam)));
